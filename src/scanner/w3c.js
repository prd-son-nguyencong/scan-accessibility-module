...
export async function scanW3cValidation(pageUrl) {
  ...
  try {
    ...
    if (validationResult === null) {
      ...
      return { url: pageUrl, violations: violations.map((v) => ({ ...v, layer: 'w3c', source: sourceOffline })), passes, timestamp: new Date().toISOString(), w3cUnavailable: true };
    }
  ...
  return { url: pageUrl, violations: dedupedViolations.map((v) => ({ ...v, layer: 'w3c', source })), passes, timestamp: new Date().toISOString() };
}
...
async function runSupplementalChecks(page, apiMessages, rawHtml) {
  const existingDescriptions = new Set(apiMessages.map((e) => e.message));
  const issues = [];
  ...
  for (const issue of domIssues) {
    const isDupe = existingDescriptions.has(issue.description);
    if (!isDupe) {
      issues.push({
        ...issue,
        wcagCriteria: '4.1.1',
